...
public class LoanQRDetail extends AppCompatActivity {
    //出库条码明细
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.loan_qr_detail);
        android.support.v7.app.ActionBar actionBar = getSupportActionBar();
        if (actionBar != null) {
            actionBar.setHomeButtonEnabled(true);
            actionBar.setDisplayHomeAsUpEnabled(true);
        }
        helper5 = new MyDataBaseHelper(LoanQRDetail.this, "ShangmiData", null, 1);
      ...
      //      current_matrname_qrRecv = _intent.getStringExtra("current_matrname_qr");
        ...
        vcooporderbcode_bText = (TextView) findViewById(R.id.vcooporderbcode_b_loanqr);
        matrnameText = (TextView) findViewById(R.id.matrname_loanqr);
        matrcodeText = (TextView) findViewById(R.id.matrcode_loanqr);
        numQRText = (TextView) findViewById(R.id.num_loanqr);
        vbillcodeText = (TextView) findViewById(R.id.vbillcode_loanqr);
        scanButton = (Button) findViewById(R.id.scan_loanqr);
        spinner = findViewById(R.id.spinner_loan_qrdetail);
        myadapter();
        scanButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!isVcooporderbcode_bAlreadyUpload()) {
                    finish();
                    Intent intent = new Intent(LoanQRDetail.this, LoanQrScanner.class);
        ...
        SaleDeliveryTableQrDetailAdapter adapter = new SaleDeliveryTableQrDetailAdapter(LoanQRDetail.this, listAllBodyPostition, mListener);
    ...
    private boolean isVcooporderbcode_bAlreadyUpload() {
        Cursor cursornew = db5.rawQuery("select uploadflag from LoanBody where pobillcode=? and itempk=?",
                new String[]{current_vbillcode_qrRecv, current_vcooporderbcode_b_qrRecv});
        if (cursornew != null && cursornew.getCount() > 0) {
            while (cursornew.moveToNext()) {
    ...
    public ArrayList<SaleDeliveryQrDetailBean> QuerySaleDeliveryBody(String current_matrcode_qrRecv) {
        ...
        Cursor cursor = db5.rawQuery("select platecode,boxcode,prodcutcode,itemuploadflag from LoanScanResult where pobillcode=? and materialcode=? and itempk=?", new String[]{current_vbillcode_qrRecv, current_matrcode_qrRecv, current_vcooporderbcode_b_qrRecv});
    ...
    private SaleDeliveryTableQrDetailAdapter.MyClickListener mListener = new SaleDeliveryTableQrDetailAdapter.MyClickListener() {
        ...
        public void myOnClick(int position, View v) {
            if (!isAlreadyUpload(listAllBodyPostition.get(position).getProdcutcode())) {
                db5.execSQL("delete from LoanScanResult where prodcutcode=?", new Object[]{listAllBodyPostition.get(position).getProdcutcode()});
    ...
    private boolean isAlreadyUpload(String prodcutcode) {
        ...
        if (cursor31 != null && cursor31.getCount() > 0) {
            Cursor cursor3 = db5.rawQuery("select itempk  from LoanScanResult where pobillcode=? and itempk=? and itemuploadflag=?",
