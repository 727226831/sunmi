...
import com.example.shanggmiqr.adapter.OtherOutgoingTableQrDetailAdapter;
...
public class OtherOutgoingQRDetail extends AppCompatActivity {
    //出库条码明细
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.other_outgoing_qr_detail);
        android.support.v7.app.ActionBar actionBar = getSupportActionBar();
        if(actionBar != null){
            actionBar.setHomeButtonEnabled(true);
            actionBar.setDisplayHomeAsUpEnabled(true);
        }
        helper5 = new MyDataBaseHelper(OtherOutgoingQRDetail.this, "ShangmiData", null, 1);
        //创建或打开一个现有的数据库（数据库存在直接打开，否则创建一个新数据库）
        //创建数据库操作必须放在主线程，否则会报错，因为里面有直接加的toast。。。
        db5 = helper5.getWritableDatabase();//获取到了 SQLiteDatabase 对象
        tableBodyListView = (ListView) findViewById(R.id.list_body_qrdetail);
        ...
        poBillCodeQRText = (TextView)findViewById(R.id.pobill_otheroutgoingqr);
        cwareNameQRText = (TextView)findViewById(R.id.cwarename_otheroutgoingqr);
        material_noQRText = (TextView)findViewById(R.id.material_no_otheroutgoingqr);
        numQRText = (TextView)findViewById(R.id.nnum_otheroutgoingqr);
        pchQRText = (TextView)findViewById(R.id.scannum_otheroutgoingqr);
        scanButton = (Button)findViewById(R.id.scan_otheroutgoingqr) ;
        scanButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(!isAlreadyUpload()){
                Intent intent = new Intent(OtherOutgoingQRDetail.this,OtherOutgoingQrScanner.class);
                ...
                intent.putExtra("chosen_line_materialcode_scanner",current_materialcode_qrRecv);
                intent.putExtra("chosen_line_nnum_scanner",String.valueOf(current_nnum_qrRecv));
                intent.putExtra("chosen_line_uploadnum_scanner",current_uploadnum_qrRecv);
                intent.putExtra("chosen_line_vcooporderbcode_b_scanner", current_vcooporderbcode_b_qrRecv);
                startActivity(intent);
                }else{
                    Toast.makeText(OtherOutgoingQRDetail.this,"已经执行发货操作的行号不允许再进行操作",Toast.LENGTH_LONG).show();
                }
            }
        });
        ...
        listAllBodyPostition = QueryOtherOutgoingBody(current_pobillcode_qrRecv,current_materialcode_qrRecv);
        ...
        OtherOutgoingTableQrDetailAdapter adapter = new OtherOutgoingTableQrDetailAdapter(OtherOutgoingQRDetail.this, listAllBodyPostition,mListener);
        tableBodyListView.setAdapter(adapter);
    }
//下面的方法内容需要根据实际更新
    public ArrayList<OtherOutgoingQrDetailBean> QueryOtherOutgoingBody(String current_pobillcodeRecv,String materialcode) {
        ArrayList<OtherOutgoingQrDetailBean> list = new ArrayList<OtherOutgoingQrDetailBean>();
        Cursor cursor = db5.rawQuery("select platecode,boxcode,prodcutcode,itemuploadflag from OtherOutgoingScanResult where pobillcode=? and materialcode=? and vcooporderbcode_b=?", new String[]{current_pobillcodeRecv,materialcode,current_vcooporderbcode_b_qrRecv});
    ...
    public boolean isAlreadyUpload() {
        Cursor cursor3 = db5.rawQuery("select pobillcode from OtherOutgoingBody where pobillcode=? and uploadflag=?",
     ...
     * 实现类，响应按钮点击事件
     */
    private OtherOutgoingTableQrDetailAdapter.MyClickListener mListener = new OtherOutgoingTableQrDetailAdapter.MyClickListener() {
        @Override
        public void myOnClick(int position, View v) {
            if(!isAlreadyUpload()){
            db5.execSQL("delete from OtherOutgoingScanResult where pobillcode=? and materialcode=? and vcooporderbcode_b=? and prodcutcode=?", new Object[] { current_pobillcode_qrRecv,current_materialcode_qrRecv,current_vcooporderbcode_b_qrRecv,listAllBodyPostition.get(position).getProdcutcode() });
            listAllBodyPostition = QueryOtherOutgoingBody(current_pobillcode_qrRecv,current_materialcode_qrRecv);
            String current_scanSum = countScannedQRCode(current_pobillcode_qrRecv,current_materialcode_qrRecv);
            insertCountOfScannedQRCode(current_scanSum);
            OtherOutgoingTableQrDetailAdapter adapter = new OtherOutgoingTableQrDetailAdapter(OtherOutgoingQRDetail.this, listAllBodyPostition,mListener);
