...
public class ProductEntryQrScanner extends AppCompatActivity {
    ...
    private boolean isCwarenameEmpty() {
        Cursor cursornew = db5.rawQuery("select cwarename from ProductEntryBody where billcode=? and itempk=?",
    ...
    private boolean isValidQr() {
        ...
        if (scannedMaccode == null || scannedMaccode.length() == 0) {
            Toast.makeText(ProductEntryQrScanner.this, "请检查物料信息及条码规则数据是否下载，或者是否为有效的条码", Toast.LENGTH_SHORT).show();
            return false;
    ...
    private String countScannedQRCode(String vbillcode, String matrcode) {
        String count = "0";
        Cursor cursor2 = db5.rawQuery("select prodcutcode from ProductEntryScanResult where billcode=? and materialcode=? and itempk=?", new String[]{vbillcode, matrcode, current_itempk_qrRecv});
